...
// Luis Utterances
postLuisIntentUtterance:(array, syn)=>{
    return axios.get('/api/setting/').then((response)=>{
        console.log('sending to luis')
    return axios.delete(`https://${response.data[0].luisRegion}.api.cognitive.microsoft.com/luis/api/v2.0/apps/${array.luisAppId}/versions/${array.luisAppVer}/examples/`,
        {
            text: syn.newSyn,
            intentName: syn.name},
           { headers: { 
